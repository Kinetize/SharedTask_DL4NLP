...
def get_datasets(datasets,
                 ...
                 do_remove_disallowed_chars=True
                 ):
    ...
    def get_embed(token):
        ...
        if use_concat_visual_char_embeddings and len(token) > 0:
            vis_embedding = embed_token_visually(
                orig_token)  # Using the orig token is crucial to notice orig perturbations
    ...
    for ds in datasets:
            ...
            for sent in text:
                ...
                for char in sent:
                    ...
                    if not char.isspace() and char not in visual_equivalents:
                        most_sim_alpha_num_char_idx = np.argmax(
                            [vce.similarity(char, anc) for anc in visual_equivalents])
        ...
        # Embed the data:
        embedded = np.array(
            [embed(remove_disallowed_chars(t) if do_remove_disallowed_chars else t) for t in loaded[1:]])
